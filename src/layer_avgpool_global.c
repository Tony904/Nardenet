	...
	if (l->in_ids.n > 1) {
		for (size_t b = 0; b < batch_size; b++) {
			...
			for (size_t a = 0; a < l->in_ids.n; a++) {
				...
				size_t inl_out_c = inl->out_c;
				size_t b_offset = bwh * inl_out_c;
				launch_forward_avgpool_global_kernel(&inl_output[b_offset], Z, (int)wh, (int)inl_out_c, 1);
				Z += inl_out_c;
			}
		}
	}
	...
	if (l->in_ids.n > 1) {
		for (size_t b = 0; b < batch_size; b++) {
			...
			for (size_t a = 0; a < l->in_ids.n; a++) {
				...
				size_t inl_out_c = inl->out_c;
				size_t b_offset = bwh * inl_out_c;
				launch_backward_avgpool_global_kernel(&inl_grads[b_offset], grads, (int)wh, (int)inl_out_c, 1);
				grads += inl_out_c;
			}
		}
	}
	else launch_backward_avgpool_global_kernel(l->in_layers[0]->gpu.grads, grads, (int)wh, (int)l->c, batch_size);
}
#else
...
void test_forward_avgpool_global(void) {
	layer l = { 0 };
	network net = { 0 };
	net.batch_size = 2;
	l.w = 2;
...
void test_backward_avgpool_global(void) {
	layer l = { 0 };
	network net = { 0 };
	net.batch_size = 2;
	l.w = 2;
