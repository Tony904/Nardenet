...
void forward_dense_cpu_gpu_compare(layer* l, network* net) {
	...
	compare_cpu_gpu_arrays(l->Z, l->gpu.Z, out_n * batch_size, l->id, "forward dense, Z, after zeroing");

	float* weights_cpu = l->weights;
	float* weights_gpu = l->gpu.weights;

	compare_cpu_gpu_arrays(l->weights, l->gpu.weights, l->n_weights, l->id, "forward dense, weights before dot products");
	...
	for (size_t b = 0; b < batch_size; b++) {
		for (size_t n = 0; n < out_n; n++) {
			...
			for (size_t a = 0; a < l->in_ids.n; a++) {
				...
				snprintf(buff, BUFFSIZE, "%s%zu", "forward dense, inl_output, pre-dot product, b=", b);
...
#pragma omp parallel for reduction(+:sum)
				...
				memset(buff, 0, sizeof(buff));
				snprintf(buff, BUFFSIZE, "%s, b=%zu, n=%zu", "forward dense, Z (dot product result), post-dot product", b, n);
	...
	if (l->batchnorm) {
		...
		compare_cpu_gpu_arrays(l->gammas, l->gpu.gammas, l->n_filters, l->id, "forward dense, gammas, pre-batchnorm");
		compare_cpu_gpu_arrays(l->biases, l->gpu.biases, l->n_filters, l->id, "forward dense, biases, pre-batchnorm");
		compare_cpu_gpu_arrays(l->means, l->gpu.means, l->n_filters, l->id, "forward dense, means, pre-batchnorm");
		compare_cpu_gpu_arrays(l->variances, l->gpu.variances, l->n_filters, l->id, "forward dense, variances, pre-batchnorm");
		compare_cpu_gpu_arrays(l->rolling_means, l->gpu.rolling_means, l->n_filters, l->id, "forward dense, rolling means, pre-batchnorm");
		compare_cpu_gpu_arrays(l->rolling_variances, l->gpu.rolling_variances, l->n_filters, l->id, "forward dense, rolling variances, pre-batchnorm");
		compare_cpu_gpu_arrays(l->Z, l->gpu.Z, l->n_filters, l->id, "forward dense, Z, pre-batchnorm");
		compare_cpu_gpu_arrays(l->Z_norm, l->gpu.Z_norm, l->n_filters, l->id, "forward dense, Z_norm, pre-batchnorm");
		...
		compare_cpu_gpu_arrays(l->gammas, l->gpu.gammas, l->n_filters, l->id, "forward dense, gammas, post-batchnorm");
		compare_cpu_gpu_arrays(l->biases, l->gpu.biases, l->n_filters, l->id, "forward dense, biases, post-batchnorm");
		compare_cpu_gpu_arrays(l->means, l->gpu.means, l->n_filters, l->id, "forward dense, means, post-batchnorm");
		compare_cpu_gpu_arrays(l->variances, l->gpu.variances, l->n_filters, l->id, "forward dense, variances, post-batchnorm");
		compare_cpu_gpu_arrays(l->rolling_means, l->gpu.rolling_means, l->n_filters, l->id, "forward dense, rolling means, post-batchnorm");
		compare_cpu_gpu_arrays(l->rolling_variances, l->gpu.rolling_variances, l->n_filters, l->id, "forward dense, rolling variances, post-batchnorm");
		compare_cpu_gpu_arrays(l->Z, l->gpu.Z, l->n_filters, l->id, "forward dense, Z, post-batchnorm");
		compare_cpu_gpu_arrays(l->Z_norm, l->gpu.Z_norm, l->n_filters, l->id, "forward dense, Z_norm, post-batchnorm");
		...
		compare_cpu_gpu_arrays(l->output, l->gpu.output, out_n * batch_size, l->id, "forward dense, output, post-activation");
	}
	else {
		// note: l->Z = l->act_inputs when batchnorm disabled
		add_biases(l->Z, l->biases, out_n, 1, batch_size);
		...
		compare_cpu_gpu_arrays(l->Z, l->gpu.Z, out_n, l->id, "forward dense, Z after adding biases");

		activate(l->Z, l->output, out_n, batch_size, l->activation, 0);
		activate(l->gpu.Z, l->gpu.output, out_n, batch_size, l->activation, 1);

		compare_cpu_gpu_arrays(l->output, l->gpu.output, out_n * batch_size, l->id, "forward dense, output after activation");
	}
	if (net->training) {
		zero_array(l->grads, out_n * batch_size);
		zero_array_gpu(l->gpu.grads, (int)(out_n * batch_size));

		compare_cpu_gpu_arrays(l->grads, l->gpu.grads, out_n * batch_size, l->id, "forward dense, grads after zeroing");
	}
}
...
void backward_dense_cpu_gpu_compare(layer* l, network* net) {
	...
	compare_cpu_gpu_arrays(grads_cpu, grads_gpu, batch_size * l->out_n, l->id, "backward dense, grads, pre-activation grads");

	get_activation_grads(l, batch_size);
	get_activation_grads_gpu(l, batch_size);

	compare_cpu_gpu_arrays(grads_cpu, grads_gpu, batch_size * l->out_n, l->id, "backward dense, grads, post-activation grads");
	...
	compare_cpu_gpu_arrays(l->bias_grads, l->gpu.bias_grads, out_n, l->id, "backward dense, bias grads");
	...
	compare_cpu_gpu_arrays(l->weights, l->gpu.weights, l->n_weights, l->id, "backward dense, weights");
	compare_cpu_gpu_arrays(l->weight_grads, l->gpu.weight_grads, l->n_weights, l->id, "backward dense, weight grads");
	...
	for (size_t b = 0; b < batch_size; b++) {
		for (size_t n = 0; n < out_n; n++) {
			...
			for (size_t a = 0; a < l->in_ids.n; a++) {
				...
				memset(buff, 0, sizeof(buff));
				snprintf(buff, BUFFSIZE, "%s, b=%zu, n=%zu", "backward dense, inl_grads, pre-math", b, n);
				...
				memset(buff, 0, sizeof(buff));
				snprintf(buff, BUFFSIZE, "%s, b=%zu, n=%zu", "backward dense, inl_outputs, pre-math", b, n);
				compare_cpu_gpu_arrays(inl_output_cpu, inl_output_gpu, inl_out_n, l->id, buff);

				memset(buff, 0, sizeof(buff));
				snprintf(buff, BUFFSIZE, "%s, b=%zu, n=%zu", "backward dense, weight grads, pre-math", b, n);
...
#pragma omp parallel for
				...
				memset(buff, 0, sizeof(buff));
				snprintf(buff, BUFFSIZE, "%s, b=%zu, n=%zu", "backward dense, weight grads, post-math", b, n);
				compare_cpu_gpu_arrays(weight_grads_cpu, weight_grads_gpu, inl_out_n, l->id, buff);

				memset(buff, 0, sizeof(buff));
				snprintf(buff, BUFFSIZE, "%s, b=%zu, n=%zu", "backward dense, inl_grads, post-math", b, n);
				compare_cpu_gpu_arrays(inl_grads_cpu, inl_grads_gpu, inl_out_n, l->id, buff);
